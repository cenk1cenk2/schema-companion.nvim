  ...
  return vim.tbl_deep_extend(
    ...
    ---@type vim.lsp.ClientConfig
    {
      ...
      on_attach = utils.add_hook_after(config.on_attach, function(client, bufnr)
        ...
        -- HACK: run on attach to get the current schema
        require("schema-companion.lsp").on_store_initialized(client.id, self)
      end),

      handlers = vim.tbl_extend("force", config.handlers or {}, {
        -- TODO: initial detection not working
        ["json/validate"] = function(_, _, req, _)
          require("schema-companion.lsp").on_store_initialized(req.client_id, self)
        end,
      }),
    }
  )
...
function M:on_update_schemas(bufnr, schemas)
  ...
  local override = vim.tbl_map(function(schema)
    return {
      fileMatch = { vim.uri_from_bufnr(bufnr) },
      uri = schema.uri,
    }
  end, schemas)
...
function M:match_schema_from_lsp(bufnr)
  ...
  local res = require("schema-companion.lsp").request_sync(client, "json/languageStatus", vim.uri_from_bufnr(bufnr)) or {}
  ...
  current_schemas = vim.tbl_map(function(schema)
    return {
      uri = schema.url,
      name = schema.name,
      description = schema.description,
