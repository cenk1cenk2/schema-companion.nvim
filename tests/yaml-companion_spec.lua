...
describe("schema detection:", function()
    ...
    it("should detect Kubernetes via CRD", function()
        assert(
            buf(
                "apiVersion: apps/v1\nkind: DaemonSet\nspec:\n  template:\n    spec:\n      containers:\n        - name:\n",
                "yaml",
        ...
        local result = { require("yaml-companion.builtin.kubernetes_crd").extractGroupVersionKind(0) }
        assert.are.same("apps", result[1])
        assert.are.same("v1", result[2])
        assert.are.same("DaemonSet", result[3])
    end)
